...
for i in tqdm(range(num_of_points)):
    ...
    m1_to_m3 = np.zeros(num_classes)
    m2_to_m3 = np.zeros(num_classes)

    for m3 in range(num_classes):
        if m1 == m3 or m2 == m3:
            continue

        case_gap = prediction[m1] - prediction[m3] - (m3 <= m1)
        deltas_case = (1 + (max_classes_given_h == m1).long() - (max_classes_given_h == m3).long()).sum(dim=1)
        m1_to_m3[m3] = get_num_of_poisoned_sample(case_gap, deltas_case)

        case_gap = prediction[m2] - prediction[m3] - (m3 <= m2)
        deltas_case = (1 + (max_classes_given_h == m2).long() - (max_classes_given_h == m3).long()).sum(dim=1)
        m2_to_m3[m3] = get_num_of_poisoned_sample(case_gap, deltas_case)
    ...
    if idx_election[i] == m1:
        for m3 in range(num_classes):
            ...
            case32_gap = prediction[m1] - prediction[m3] - (m3 <= m1) + prediction[m1] - prediction[m2] - (m2 <= m1)
            deltas_case32 = (1 + 2 * (max_classes_given_h == m1).long() - (max_classes_given_h == m2).long() - (max_classes_given_h == m3).long()).sum(dim=1)
            n2 = get_num_of_poisoned_sample(case32_gap, deltas_case32)
...
torch.save(certs,'./radii/fa_election_'+args.evaluations+'_improved.pth')
